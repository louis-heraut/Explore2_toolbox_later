...
for (file in Files) {
    ...
    cols = names(data_tmp)[2:ncol(data_tmp)]
    ...
    data_tmp = relocate(data_tmp, SH, .before="deltaRecharge")
    data_tmp = relocate(data_tmp, GWL, .before=GCM)
    data_tmp = relocate(data_tmp, EXP, .before=GCM)
    data_tmp = relocate(data_tmp, BC, .before=SH)
    data_tmp = relocate(data_tmp, HM, .before=SH)

    data = bind_rows(data, data_tmp)
}

outfile =
    "deltaRecharge_yr_GWL-all_historical-rcp85_all_all_ADAMONT_all_on-secteur-hydro.csv"
